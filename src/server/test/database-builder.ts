...
export async function createIsolatedDatabase(options?: { readonly throughVersion?: string }): Promise<IsolatedDatabase> {
  ...
  try {
    const migrationNames = readdirSync(join(process.cwd(), 'drizzle'))
      .filter((entry) => /^\d{4}_.+\.sql$/.test(entry))
      .filter((entry) => !options?.throughVersion || entry.slice(0, 4) <= options.throughVersion)
      .sort();
