...
export class TaskWorkApplication {
    ...
    async transition(input: TaskTransitionInput): Promise<Result<TaskStatusTransition>> {
        ...
        return this.transaction(async (db, time) => {
            ...
            if (!taskAcceptsTime({ status: 'in_progress', reviewState: task.review_state, assigneeEmployeeId: task.assignee_employee_id })) return invalid('taskId', 'Wait for Team Lead review before starting or completing this task.', 'TASK_AWAITING_REVIEW');
