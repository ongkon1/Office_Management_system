...
describe('Backend Phase 3 organization and work rules',()=>{
  ...
  it.each(['not_required','pending_review','rejected'] as const)('refuses time for a self-raised task in %s state',async(state)=>{const r=new MemoryRepository();r.projects.set('p1',project);r.tasks.set('t1',{...task(state),status:'in_progress'});const result=await new OrganizationWorkService(r,effects).validateTaskForTime('t1');expect(result.status).toBe(state==='not_required'?'success':'validation_failure')});
  it('allows time only after approval',async()=>{const r=new MemoryRepository();r.projects.set('p1',project);r.tasks.set('t1',{...task('approved'),status:'in_progress'});expect((await new OrganizationWorkService(r,effects).validateTaskForTime('t1')).status).toBe('success')});
  ...
  it('forces a Team Lead self-task to the authenticated lead with no review required',async()=>{const r=new MemoryRepository();r.projects.set('p1',project);const s=new OrganizationWorkService(r,effects);const result=await s.saveTask(leadActor,{...task('approved'),creatorEmployeeId:'other',assigneeEmployeeId:'lead',reviewerEmployeeId:'other'});expect(result.status).toBe('success');if(result.status==='success'){expect(result.data.creatorEmployeeId).toBe('lead');expect(result.data.assigneeEmployeeId).toBe('lead');expect(result.data.reviewState).toBe('not_required');expect(result.data.reviewerEmployeeId).toBeNull()}});
