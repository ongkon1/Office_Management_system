...
import type { WorkPolicy } from '@/contracts/domain';
...
const policy: WorkPolicy = { ...audit, id: 'policy', name: 'Standard', version: 1, effectiveFrom: '2026-01-01', effectiveTo: null, requiredActiveMinutes: 420, recognizedBreakMinutes: 60, requiredTotalMinutes: 480, overtimeThresholdMinutes: 480, criticalThresholdMinutes: 720, workingWeekdays: [7, 1, 2, 3, 4], businessTimezone: 'Asia/Dhaka' };
const entry = (minutes: number, divisionId = 'pia'): WorkLog => ({ ...audit, id: divisionId, employeeId: 'employee', workDate: '2026-09-02', divisionId, projectId: 'project', taskId: 'task', source: 'manual', idempotencyKey: divisionId, workLocation: 'office', durationMinutes: minutes, workDescription: 'Work', completedWork: 'Done', supportingLink: null, attachmentIds: [], state: 'saved', overtimeReason: null, criticalExplanation: null, policyVersion: 1 });
describe('BE-0450..0454 canonical calculations and timezone boundaries', () => {
    it.each([[0, 'missing'], [419, 'under_time'], [420, 'complete'], [421, 'overtime'], [660, 'overtime'], [661, 'critical']] as const)('classifies %i active minutes as %s', (minutes, status) => { expect(calculateDay({ employeeId: 'employee', workDate: '2026-09-02', policy, workLogs: minutes ? [entry(minutes)] : [] }).status).toBe(status); });
    it('requires both normal thresholds', () => { expect(calculateDay({ employeeId: 'employee', workDate: '2026-09-02', policy, workLogs: [entry(420)], breakOverrideMinutes: 59 }).status).toBe('under_time'); });
    it('reconciles 3+2+2 hours and exactly one break across divisions', () => { const s = calculateDay({ employeeId: 'employee', workDate: '2026-09-02', policy, workLogs: [entry(180, 'pia'), entry(120, 'gov'), entry(120, 'west')] }); expect(s).toMatchObject({ activeMinutes: 420, breakMinutes: 60, totalMinutes: 480, status: 'complete' }); expect(s.divisionContributions.reduce((n, c) => n + c.activeMinutes, 0)).toBe(420); });
    it('uses half-day leave consistently in validation and calculation', () => { const draft = { employeeId: 'employee', workDate: '2026-09-02', divisionId: 'pia', projectId: null, taskId: null, entryMethod: 'manual_duration' as const, activeMinutes: 450, workLocation: 'office', startTime: null, endTime: null, workDescription: 'Work', completedWork: 'Done', overtimeReason: null, criticalExplanation: null }; const leave = { portion: 'half_day' as const, leaveType: 'annual' }; expect(validateEntry(draft, { policy, existingEntries: [], effectiveDivisionIds: ['pia'], projects: [], tasks: [], leave })).toEqual([]); expect(calculateDay({ employeeId: 'employee', workDate: '2026-09-02', policy, workLogs: [entry(210)], leave })).toMatchObject({ activeMinutes: 210, breakMinutes: 30, totalMinutes: 240, status: 'complete' }); });
